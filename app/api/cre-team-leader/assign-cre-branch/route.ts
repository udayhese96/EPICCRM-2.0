...
export async function PUT(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${process.env.FASTAPI_URL || 'http://localhost:8000'}/api/cre-team-leader/assign-cre-branch`, {
      method: 'PUT',
      headers: {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({ error: 'Failed to assign CRE to branch' }, { status: response.status })
    }

    const data = await response.json()
    return NextResponse.json(data)
  } catch (error) {
    console.error('Error assigning CRE to branch:', error)
