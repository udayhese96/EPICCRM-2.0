...
async function callFastAPIWithRetry(branch: string, authToken: string, dateFilterType?: string, startDate?: string, endDate?: string, teamLeader?: string, retries = MAX_RETRIES): Promise<Response> {
  ...
  try {
    ...
    const url = `${FASTAPI_BASE_URL}/analytics/sales-manager/ps-performance?${params.toString()}`
    ...
    try {
      ...
      if (!response.ok && retries > 0) {
        ...
        return callFastAPIWithRetry(branch, authToken, dateFilterType, startDate, endDate, teamLeader, retries - 1)
      }
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (dateFilterType === 'from_to') {
      ...
      console.log('📅 API Route - Date filter validation:', {
        dateFilterType,
      ...
      if (!normalizedStartDate || !normalizedEndDate) {
        console.error('❌ API Route - Missing dates:', { normalizedStartDate, normalizedEndDate })
        return NextResponse.json({ 
          error: 'start_date and end_date are required for \'from_to\' filter. Please select both start and end dates.',
          success: false,
          details: {
            startDate: normalizedStartDate || 'MISSING',
            endDate: normalizedEndDate || 'MISSING'
          }
        }, { status: 400 })
      }
      ...
      console.log('📅 API Route - Date parsing:', {
        normalizedStartDate,
        normalizedEndDate,
      ...
      if (isNaN(startDateObj.getTime()) || isNaN(endDateObj.getTime())) {
        console.error('❌ API Route - Invalid date format:', { normalizedStartDate, normalizedEndDate })
        return NextResponse.json({ 
          error: 'Invalid date format. Please provide dates in YYYY-MM-DD format.',
          success: false,
          details: {
            receivedStartDate: normalizedStartDate,
            receivedEndDate: normalizedEndDate
          }
        }, { status: 400 })
      }
      ...
      if (endDateObj < startDateObj) {
        console.error('❌ API Route - Invalid date range:', { normalizedStartDate, normalizedEndDate })
        return NextResponse.json({ 
      ...
      console.log('✅ API Route - Date validation passed:', { normalizedStartDate, normalizedEndDate })
    }
    ...
    const authToken = authHeader.substring(7) // Remove 'Bearer ' prefix
                ...
                if (teamLeader) {
                  console.log(`🔄 Team Leader filter: ${teamLeader}`)
                }

                // Call FastAPI backend with retry logic and authentication
                const fastApiResponse = await callFastAPIWithRetry(branch, authToken, dateFilterType || undefined, startDate || undefined, endDate || undefined, teamLeader || undefined)

    if (!fastApiResponse.ok) {
      let errorMessage = 'Failed to fetch PS performance data from backend'
    ...
    console.log(`✅ Successfully fetched PS performance data for branch: ${branch}`)
    console.log(`✅ Team Leaders in response:`, result.team_leaders)
    console.log(`✅ Team Leaders count:`, result.team_leaders?.length || 0)
    
                return NextResponse.json({ 
                  data: result.data || [], 
                  kpi_cards: result.kpi_cards || {},
                  ps_rankings: result.ps_rankings || [],
                  team_leaders: Array.isArray(result.team_leaders) ? result.team_leaders : [],
    ...
    console.error('❌ PS performance API error:', error)
