...
async function callFastAPIWithRetry(branch: string, authToken: string, retries = MAX_RETRIES): Promise<Response> {
  try {
    const response = await fetch(`${FASTAPI_BASE_URL}/analytics/sales-manager/source-analytics?branch=${encodeURIComponent(branch)}`, {
      method: 'GET',
      headers: {
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(`🔄 Fetching Source Analytics data for branch: ${branch}`)
    ...
    if (!fastApiResponse.ok) {
      let errorMessage = 'Failed to fetch Source Analytics data from backend'
    ...
    console.log(`✅ Successfully fetched Source Analytics data for branch: ${branch}`)
    ...
    console.error('❌ Source Analytics API error:', error)
