...
export async function GET(request: NextRequest) {
  try {
    ...
    if (offset !== '0' && !cursor) {
      console.warn('[Todays Followup] Using offset pagination - consider migrating to cursor')
    }
    ...
    if (!response.ok) {
      ...
      return respondError("Failed to fetch today's follow-up data", response.status, errorText)
    }
