...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await fetch(
      `${FASTAPI_BASE_URL}/api/team-leader/waiting-approval?${backendParams.toString()}`,
      {
        method: 'GET',
        headers: {
    ...
    if (!response.ok) {
      ...
      console.error('[WaitingApproval API] Backend error:', response.status, errorText)

      return NextResponse.json(
        {
          error: 'Failed to fetch waiting approval leads data',
          details: errorText
        },
    ...
    console.error('[WaitingApproval API] Error:', error)
    return NextResponse.json(
      {
