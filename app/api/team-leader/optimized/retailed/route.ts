...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('[Optimized Retailed] Params:', { teamLeaderId, dateRange, psMember, startDate, endDate })
    ...
    const response = await fetch(
      `${FASTAPI_BASE_URL}/api/team-leader/optimized/retailed?${backendParams.toString()}`,
      {
        method: 'GET',
        headers: {
    ...
    if (!response.ok) {
      ...
      console.error('[Optimized Retailed API] Backend error:', response.status, errorText)

      return NextResponse.json(
        {
          error: 'Failed to fetch retailed data',
          details: errorText
        },
    ...
    console.error('[Optimized Retailed API] Error:', error)
    return NextResponse.json(
      {
