...
// Optimized Today Follow-up API Route
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('[Optimized Today Follow-up] Params:', { teamLeaderId, dateRange, psMember, startDate, endDate })
    ...
    const response = await fetch(
      `${FASTAPI_BASE_URL}/api/team-leader/optimized/todays-followup?${backendParams.toString()}`,
      {
        method: 'GET',
        headers: {
    ...
    if (!response.ok) {
      ...
      console.error('[Optimized Today Follow-up API] Backend error:', response.status, errorText)

      return NextResponse.json(
        {
          error: 'Failed to fetch today follow-up data',
          details: errorText
        },
    ...
    console.error('[Optimized Today Follow-up API] Error:', error)
    return NextResponse.json(
      {
