...
// Optimized Open Leads API Route
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('[Optimized Open Leads] Params:', { teamLeaderId, dateRange, psMember, startDate, endDate })
    ...
    const response = await fetch(
      `${FASTAPI_BASE_URL}/api/team-leader/optimized/open-leads?${backendParams.toString()}`,
      {
        method: 'GET',
        headers: {
    ...
    if (!response.ok) {
      ...
      console.error('[Optimized Open Leads API] Backend error:', response.status, errorText)
    ...
    console.error('[Optimized Open Leads API] Error:', error)
    return NextResponse.json(
      {
