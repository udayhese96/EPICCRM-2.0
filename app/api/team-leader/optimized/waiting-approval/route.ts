...
// Optimized Waiting Approval API Route
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('[Optimized Waiting Approval] Params:', { teamLeaderId, dateRange, psMember, startDate, endDate })
    ...
    const response = await fetch(
      `${FASTAPI_BASE_URL}/api/team-leader/optimized/waiting-approval?${backendParams.toString()}`,
      {
        method: 'GET',
        headers: {
    ...
    if (!response.ok) {
      ...
      console.error('[Optimized Waiting Approval API] Backend error:', response.status, errorText)

      return NextResponse.json(
        {
          error: 'Failed to fetch waiting approval data',
          details: errorText
        },
    ...
    console.error('[Optimized Waiting Approval API] Error:', error)
    return NextResponse.json(
      {
