...
const SourceAnalyticsTable: React.FC<SourceAnalyticsTableProps> = ({ branch }) => {
  ...
  const handleCellClick = async (sourceName: string, isMainSource: boolean, parentSource?: string) => {
    // Don't allow clicking on cells with 0 count
    const row = data.find(r => r.source === sourceName)
    if (!row || row.count === 0) return
    
    try {
      setDrillDownLoading(true)
      const displaySource = isMainSource ? sourceName : (parentSource ? `${parentSource} - ${sourceName}` : sourceName)
      setDrillDownTitle(`${displaySource} (${row.count} leads)`)
      ...
      // For sub-sources, pass sub_source parameter
      const params = new URLSearchParams({
        branch: currentBranch,
        source: isMainSource ? sourceName : (parentSource || sourceName)
      })
      if (!isMainSource && sourceName) {
        params.append('sub_source', sourceName)
      }
      
      const response = await fetch(
        `/api/analytics/sales-manager/source-leads?${params.toString()}`,
        {
          cache: 'no-store',
          headers: {
            'Authorization': `Bearer ${token}`,
          ...
          <Table className="w-full text-xs">
            ...
            <TableBody>
              {data.map((row, index) => (
                ...
                >
                  <TableCell 
                    className={`text-left ${row.count > 0 ? 'cursor-pointer hover:bg-gray-50' : ''}`}
                    onClick={() => row.count > 0 && handleCellClick(row.source, row.is_main_source, row.parent_source)}
                  >
                  ...
                  <TableCell 
                    className={`text-center ${row.count > 0 ? 'cursor-pointer hover:bg-gray-50' : ''}`}
                    onClick={() => row.count > 0 && handleCellClick(row.source, row.is_main_source, row.parent_source)}
                  >
                    {row.count}
                  </TableCell>
                    ...
                    <Button
                      onClick={() => {
                        try {
                          ...
                          const headers = ['Lead UID', 'PS Name', 'Customer Name', 'Mobile', 'Alternate Mobile', 'Source', 'Sub-Source', 'CRE', 'Model Interested', 'Status', 'First Call Date', 'Second Call Date', 'Third Call Date', 'Fourth Call Date', 'Fifth Call Date', 'Sixth Call Date', 'Seventh Call Date']
                ...
                <div className="space-y-6">
                  ...
                  <Card>
                    ...
                    <CardContent className="grid grid-cols-2 gap-4">
                      ...
                      {selectedLead.sub_source && (
                        <div>
                          <label className="text-sm font-semibold text-gray-500">Sub-Source</label>
                          <div className="text-sm">{selectedLead.sub_source || 'N/A'}</div>
                        </div>
                      )}
                      <div>
